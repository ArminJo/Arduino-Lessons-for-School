 ...
 * EncoderMotor.cpp
 *
 *  Functions for controlling a DC-motor which rotary encoder implemented by fork light barrier and an attached encoder disc.
...
bool EncoderMotor::updateMotor() {
    ...
    if (State == MOTOR_STATE_FULL_SPEED) {
        ...
        if (EncoderCount >= NextChangeMaxTargetCount) {
             ...
             */
            RampDeltaPerDistanceCount = ((CurrentSpeed - MinSpeed) * 2) / ((TargetDistanceCount - EncoderCount)) + 1;
            // brake
...
void EncoderMotor::resetControlValues() {
    memset(&CurrentMaxSpeed, 0, (((uint8_t *) &Debug) + sizeof(Debug)) - &CurrentMaxSpeed);
