...
#  endif
        ...
        if (sStepMode != MODE_SINGLE_STEP) {
            ...
            insertToPath(rightMotor.LastRideEncoderCount, sLastDegreesTurned, true);
        }
#endif
    ...
    if (sDoStep || sStepMode == MODE_CONTINUOUS || (sStepMode == MODE_STEP_TO_NEXT_TURN && (!RobotCarMotorControl.isStopped()))) {
        ...
        if (sStepMode == MODE_SINGLE_STEP) {
            ...
            if (sNextDegreesToTurn == GO_BACK_AND_SCAN_AGAIN) {
                ...
                // and go
                RobotCarMotorControl.goDistanceCentimeter(CENTIMETER_PER_RIDE,DIRECTION_FORWARD, &loopGUI);
            }
         ...
         */
#ifdef USE_ENCODER_MOTOR_CONTROL
        uint16_t tStepStartDistanceCount = rightMotor.EncoderCount; // get count before distance scanning
#endif
        ...
        if (!RobotCarMotorControl.isStopped()) {
             ...
             */
#ifdef USE_ENCODER_MOTOR_CONTROL
            sCentimeterPerScanTimesTwo = rightMotor.EncoderCount - tStepStartDistanceCount;
        ...
        if ((sNextDegreesToTurn != 0 && sStepMode == MODE_STEP_TO_NEXT_TURN) || sStepMode == MODE_SINGLE_STEP) {
             ...
             */
            insertToPath(rightMotor.EncoderCount, sLastDegreesTurned, false);
            rightMotor.synchronizeMotor(&leftMotor, MOTOR_DEFAULT_SYNCHRONIZE_INTERVAL_MILLIS);
#endif
        }
