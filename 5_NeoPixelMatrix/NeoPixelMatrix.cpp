...
void setup() {
	...
	for (unsigned int i = 0; i < NEOPIXEL_MATRIX_NUM_COLUMNS; i++) {
		...
		NeoPixelMatrix.setMatrixPixelColor(i, 0, NeoPixelMatrix.Color(tValue, 0, 0)); // Red
	}
	NeoPixelMatrix.show();
	delay(4000);
...
void loop() {
	 ...
	 */
	for (unsigned int i = 0; i < NEOPIXEL_MATRIX_NUM_COLUMNS; i++) {
		NeoPixelMatrix.setPixelColor(i, Wheel(tWheelOffset + (tWheelOffset + (i * (256 / NEOPIXEL_MATRIX_NUM_COLUMNS)))));
	}
	NeoPixelMatrix.show();
	tWheelOffset++;
